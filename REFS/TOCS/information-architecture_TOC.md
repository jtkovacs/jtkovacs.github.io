...
Terms can be generated by a committee (top-down, bottom-up); by empirical methods (deductive, inductive); by a machine; or from an existing CV (don't duplicate!). Terms are chosen from a specific domain/information space if their inclusion is warranted. Since warrants shift over time, a taxonomist may also have a 'parking lot' for candidate terms AKA provisional terms:
